...
class DDQNAgent:
    ...
    """
    def __init__(self, input_dim, num_actions, agent_id, learning_rate=0.00025, gamma=0.99, batch_size=64, buffer_capacity=100000, update_target_freq=1000, tau=0.001, learning_rate_decay=0.000002):
        """
        ...
        """
        self.id = agent_id  # ID des Agenten
    ...
    def communicate(self, other_agent_index, found_target, target_pos=None):
        """
        Lässt den Agenten mit anderen Agenten kommunizieren.

        Args:
            other_agent_index (int): Der Index des anderen Agenten.
            found_target (bool): Ob der Agent das Ziel gefunden hat.
            target_pos (tuple): Die Position des Ziels, wenn das Ziel gefunden wurde.
        """
        if found_target:
            print(f"Agent {self.id} meldet an Agent {other_agent_index}: Ziel gefunden bei {target_pos}.")
        else:
            print(f"Agent {self.id} meldet an Agent {other_agent_index}: Ziel noch nicht gefunden.")
    ...
    def load(self, filepath, weights_only=True):
        """
        Lädt das Modell.

        Args:
            filepath (str): Der Pfad zur Ladedatei.
            weights_only (bool): Ob nur die Gewichte geladen werden sollen.
        """
...
class Environment:
    ...
    def step(self, actions):
        ...
        for agent_index, action in enumerate(actions):
            ...
            # Setzen Sie den `new_x` und `new_y` auf die aktuelle Position, um sie bei Bedarf zu ändern
            new_x, new_y = x, y  # Dies sorgt dafür, dass `new_x` und `new_y` immer definiert sind.
            ...
            if action == 0:  # Norden
                ...
                # Sammeln tut nichts an der Position, es sei denn, es gibt Ressourcen
            ...
            # **Prüfung auf Hindernisse oder blockierte Ressourcen**
            if self.grid[new_x, new_y] == 5:  # Hindernis blockiert den Weg
                collision_reward = REWARD_COLLISION
            elif self.grid[new_x, new_y] in RESOURCE_TYPES:  # Prüfen, ob die Ressource den Weg blockiert
                collision_reward = REWARD_COLLISION
            else:
                # Bewegung ist frei von Hindernissen und blockierenden Ressourcen
